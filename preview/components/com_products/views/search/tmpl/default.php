...
<a name="filter" ></a>
		...
		<div class="mt">
				<h1>“<?php echo $_GET['k'];?>”</h1><strong> 搜索结果(<?php echo $this->lists['nav']->totalFile;?>)</strong>
			...
			<ul id="proslab">
				...
				foreach( $rows as $k=>$v ){
					...
					<li>
						...
						<a href="<?php echo $uri->getByURL(array($f_key=>''));?>#filter" <?php if( $f == '' ){ echo ' class="now" '; }?> >全部</a>  |  
						<?php
						foreach( $values as $val ){
							$val = trim($val);
							
						?>
							<a href="<?php echo $uri->getByURL(array($f_key=>$val));?>#filter" <?php if( $f == $val ){ echo ' class="now" '; }?> >
							<?php echo $val; ?>
							</a>
						<?php
						}
...
?>
		...
		<div id="pdshow" style="margin-top:5px;" >
