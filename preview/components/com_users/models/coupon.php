...
class CouponModel extends Model
{
	...
	function getList(){
		...
		$sql = " select f.*,p.name,p.catid  from ".$this->tableName." as f ";
		$sql .=" left join #__products as p on f.product_id=p.id ";
		$sql .= $where;
		$sql .= $order;
		...
		//没有评论的产品信息
		$sql =" select product_name,product_id from #__order_goods where uid=".$uid." and iscomment = 0 order by product_id desc ";
		$this->db->query($sql);
		$lists['goods'] = $this->db->getResult();
		return  $lists;
	}
