...
if( empty($byTime) ){ $byTime='oneWeek'; }
 ...
 ?>	 
 <div class="right_top" >
<h2  >我的团购</h2><span class="goon_btn"><a title="首页" href="./" style="color: rgb(255, 255, 255);">继续购物</a></span>
</div> 
		...
		**/ ?>
		...
		<table width="100%" cellspacing="0" cellpadding="0" border="0"  class="u_tab" >
				<tbody><tr>
					...
					<th  width="380" bgcolor="#FFFFFF" >订单信息/团购商品</th> 
				...
				if( $num > 0 ){
					...
					foreach( $rows as $k=>$r  ){
						...
						<tr <?php if( $i++ < $num ){ echo ' class="btmbd" '; }else{ } ?> >
							...
							<td   >
								<a href="index.php?com=users&view=tuan&act=view&id=<?php echo $row['id'];?>">查看
								</a> 
								<br/>
								<?php
...
function cancel(id){
	var href="index.php?com=users&view=tuan&act=cancel&id="+id;
	if( confirm('确认取消订单吗！') ){
		location.href=href;
	}
