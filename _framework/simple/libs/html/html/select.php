...
class WHTMLSelect
{
	...
	function options( $arr, $key = 'value', $text = 'text', $selected = null, $translate = false )
	{
		...
		foreach ($arr as $i => $option)
		{
			...
			else
			{
				...
				$k = WFilterOutput::ampReplace($k);
				$t = WFilterOutput::ampReplace($t);
	 ...
	 * @param	string	The name of the object variable for the option value  值对应的字段名
	 * @param	string	The name of the object variable for the option text	  选择列表显示的文本框
	 * @param	mixed	The key that is selected (accepts an array or a string)
	 * @returns	string	HTML for the select list
	 */
	function genericlist( $arr, $name, $attribs = null, $key = 'value', $text = 'text', $selected = NULL, $idtag = false, $translate = false )
	{
		...
		$html	.= WHTMLSelect::Options( $arr, $key, $text, $selected, $translate );
		$html	.= '</select>';
	...
	*/
	function integerlist( $start, $end, $inc, $name, $attribs = null, $selected = null, $format = "" )
	{
		...
		for ($i=$start; $i <= $end; $i+=$inc)
		{
			...
			$arr[] = WHTML::_('select.option',  $fi, $fi );
		}

		return WHTML::_('select.genericlist',   $arr, $name, $attribs, 'value', 'text', $selected );
	}
	...
	*/
	function radiolist( $arr, $name, $attribs = null, $key = 'value', $text = 'text', $selected = null, $idtag = false, $translate = false )
	{
		...
		{
			$k	= $arr[$i]->$key;
			$t	= $translate ? WText::_( $arr[$i]->$text ) : $arr[$i]->$text;
	...
	*/
	...
	{
		$arr = array(
			WHTML::_('select.option',  '0', WText::_( $no ) ),
			WHTML::_('select.option',  '1', WText::_( $yes ) )
		);
		return WHTML::_('select.radiolist',  $arr, $name, $attribs, 'value', 'text', (int) $selected, $id );
	}
}
