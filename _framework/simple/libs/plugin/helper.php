...
class PluginHelper
{
	...
	*/
	function importPlugin($type, $plugin = null, $autocreate = true, $dispatcher = null)
	{
		...
		for($i = 0; $i < $total; $i++) {
			if($plugins[$i]->type == $type && ($plugins[$i]->name == $plugin ||  $plugin === null)) {
				PluginHelper::_import( $plugins[$i], $autocreate, $dispatcher );
				$result = true;
			}
		}
	...
	function _import( &$plugin, $autocreate = true, $dispatcher = null )
	{
		...
		$path	= PATH_PLUGINS.DS.$plugin->type.DS.$plugin->name.'.php';
			...
			if (file_exists( $path ))
			{
				...
				if($autocreate)
				{
					...
					if(class_exists($className))
					{
						...
						$plugin =& PluginHelper::getPlugin($plugin->type, $plugin->name);
	...
	function _load()
	{
		...
		if (!($plugins = $db->loadObjectList())) {
			Error::raiseWarning( 'SOME_ERROR_CODE', "Error loading Plugins: " . $db->getErrorMsg());
			return false;
		}
