...
function uploadSuccess(file, serverData) {
	...
	//alert( serverData );
	var s = serverData.substring(0,2);
	//上传成功!
	if( s == 's:' ){
		try {
			var progress = new FileProgress(file, this.customSettings.progressTarget);
			...
			progress.toggleCancel(false); 
			serverData = serverData.substring(2); 
			var pos = serverData.indexOf(':'); 
			//var name = serverData.substring(3,pos)
		
			//alert($('input[name=iscom]').val());
			if( parent.addpimg && $('input[name=iscom]').val() == 'products'  ){
				parent.addpimg(serverData.substring(0,pos),serverData.substring(pos+1));
			}else{
				window.parent.uploadSuccess(serverData.substring(pos+1),serverData.substring(0,pos));
			}
		...
		try {
			var progress = new FileProgress(file, this.customSettings.progressTarget);
			progress.setError();
			progress.setStatus("<font color=red >上传失败.</font>");
			alert(serverData);
			//progress.toggleCancel(false);
...
function queueComplete(numFilesUploaded) {
	...
	status.innerHTML = "已上传 "+numFilesUploaded + " 个文件" + (numFilesUploaded === 1 ? "" : " ") + ".";

	setTimeout(function(){  parent.$.w.closeN(2);},200); 
}
