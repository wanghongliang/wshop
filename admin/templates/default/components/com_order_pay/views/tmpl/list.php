...
<table class="listtable"  >
	<thead>
		<tr>
			...
			<th>
			<?php 
				echo HTML::_('grid.sort',   '收款账号', 'c.account', $lists['order_dir'], $lists['order'] ); 
			?>
			
			</th>
			<th>
			<?php 
				echo HTML::_('grid.sort',   '收款银行', 'c.bank', $lists['order_dir'], $lists['order'] ); 
			?>
			...
			<th>
			<?php 
				echo HTML::_('grid.sort',   '完成时间', 'c.order_status', $lists['order_dir'], $lists['order'] ); 
			?>
 	...
 	if( is_array($this->rows) ){
		...
		?>
			<tr class="trbg<?echo $i;?> <?php if( $row['pay_status'] == '0' ){ echo 'nopay'; } ?>" >
				<td>
					<input type="checkbox" name="id[]" value="<?php echo $row['payment_id'];?>" class="ids" />
				</td>
				...
				<td>支付成功</td>
				<td><?php echo date('Y-m-d H:i:s',$row['t_begin']);?></td>


 				<td>
				<a href="javascript:;;" class="v" url="<?php echo $this->baseuri;?>&tmpl=component&task=edit&id=<?php echo $row['payment_id'];?>" >
				查看
				</a>

				&nbsp;
				<a href="javascript:del('<?php echo $this->baseuri;?>&task=del&id=<?php echo $row['payment_id'];?>');" >
				删除
				</a>
 ...
 $(function(){
	 $('.v').wDialog({title:'查看收款单信息',width:800,height:500,top:30,iframe:true});
 });
