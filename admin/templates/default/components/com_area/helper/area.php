...
class AreaHelper
{
	...
	*/
	function addsort($CatagoryID,$data)
	{
		...
		$sql= "INSERT INTO `".$this->table."` SET `lft`='$rgt',`rgt`='$rgt'+1, `parent_id`='$parent_id', `depth`='$depth' ".$this->db->arrayToField($data);
	...
	*/
	function movecatagory($SelfCatagoryID,$ParentCatagoryID,$data)
	{
		...
		if($Parentrgt>$Selfrgt){	//当向后移动时
			...
			$data['depth'] = $NewCatagory['depth']+1;
			...
			$middleLeftSQL = "UPDATE `".$this->table."` SET `lft`=`lft`+$Value ".$this->bWhere("  and `lft`>$Parentrgt and  id NOT IN($InIDS)");
			$middleRightSQL = "UPDATE `".$this->table."` SET `rgt`=`rgt`+$Value ".$this->bWhere("  `rgt`>=$Parentrgt and id NOT IN($InIDS)");
