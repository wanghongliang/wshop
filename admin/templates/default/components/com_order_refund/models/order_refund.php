...
{ 
	var $nav = null;
	function Order_refundModel()
	{
		parent::__construct();
		$this->tableName = '#__refunds ';
 	}
	...
	function getList()
	{
		...
		if( $order )
		{
			...
			$order = " order by c.refund_id desc ";
		}
	...
	function getItem()
	{
		...
		if( $id> 0 ){
			$sql = "select * from ".$this->tableName." as o   where o.refund_id=".$id;
 			$this->db->query($sql);
	...
	function delete($id)
	{
		if( $id > 0 )
		{
			...
			$sql = "delete from ".$this->tableName." where refund_id=".$id;
			$this->db->query($sql); 
			return true;
		}
