...
<form action="<?php echo $this->baseuri;?>"  method="post"  id="menage_form" name="listform"   enctype="multipart/form-data"  >
	...
	<table class="formtable" width="100%" id="group-table"    >
 		...
 		<tr  >
			<td class="form_text" >自定义团购图片:</td>
			<td> 
				<input type="file" name="uploadimg"  id="uploadimg" value=""  />
				<div class="help" >
					上传自定义的团购图片,保持宽度为420px为佳,宽度280px.
				</div>
			</td>
		</tr>
 		<tr  >
			<td class="form_text" >团购标题:</td>
			<td> 
				<textarea name="act_name" id="act_name" cols=50 rows=6 ><?php echo $item['act_name'];?></textarea> 
			</td>
		</tr>
		...
		?>
  		<tr  >
			<td class="form_text" >保证金:</td>
			<td>
				<input type="text" name="deposit" size=30 value="<?php echo $param['deposit'];?>"  />
			</td>
		</tr>

  		<tr  >
			<td class="form_text" >限购数量:</td>
			<td>
				<input type="text" name="restrict_amount" size=30 value="<?php echo $param['restrict_amount'];?>"  />
				<div class="help" >
  		...
  		<tr  >
			...
			<?php
			if( ($n = count($param['ladder_amount']) )>0 ){
				for($i=0;$i<$n;$i++ ){

					if( $i== 0 ){
						?>
					<tr  >
						<td class="form_text" >价格阶梯:</td>
						<td>
						<?php
					}else{
						?>
			...
			?>
			 数量达到 <input name="ladder_amount[]" value="<?php echo $param['ladder_amount'][$i];?>" size="8" type="text">&nbsp;&nbsp;
			 享受价格 <input name="ladder_price[]" value="<?php echo $param['ladder_price'][$i];?>" size="8" type="text">
		...
		<tr  >
			<td class="form_text" >价格阶梯:</td>
			<td>
			 数量达到 <input name="ladder_amount[]" value="" size="8" type="text">&nbsp;&nbsp;
			 享受价格 <input name="ladder_price[]" value="" size="8" type="text">
			 <a href="javascript:;" onclick="addLadder(this)"><strong>[+]</strong></a>  
			 </td>
		</tr> 
			<?php } ?>
	  	<tr  >
			<td class="form_text" >已经购买的人数:</td>
			<td>
				<input type="text" name="purchase_people" size=30 value="<?php echo $item['purchase_people'];?>"  /> 
			</td>
		</tr>

	  	<tr  >
			<td class="form_text" >描述:</td>
			<td>
				<textarea name="act_remark" id="act_remark" cols=80 rows=3 ><?php echo $item['act_remark'];?></textarea> 
			</td>
		</tr>
		<tr  >
			...
			echo $editor->display('act_desc',$item['act_desc'],'100%','450');
			?> 
 			</td>
		</tr>
