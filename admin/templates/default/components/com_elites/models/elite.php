...
class EliteModel extends Model
{
	function EliteModel()
	{
		parent::__construct();
	...
	function delete($id)
	{
		...
		$s=(int)$_REQUEST['s'];
		$id=(int)$_REQUEST['id'];
		$sql=" delete from ".$this->tableName." where  elite_id=".$s." and products_id='".$id."' ";
		$this->db->query($sql);
	...
	function deleleall(){
		...
		$s=(int)$_REQUEST['s'];
 		if( count($ids) )
		{
			$sql = " delete from ".$this->tableName." where elite_id=$s and products_id in (".implode(',',$ids).") ";
			//echo $sql;exit;
			$this->db->query($sql);
 
 			$app->enqueueMessage(' 删除成功,共删除 '.count($ids).'项.');
		}
		return true;
	}
	/**
	...
	function ordering()
	{
 		...
 		if( $id>0 && $to>0 && $s>0 )
		{
			...
			$this->db->updateArray($this->tableName,$data," elite_id=".$s." and products_id=".$id." ");
