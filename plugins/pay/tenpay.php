 ...
 * 在线支付方式
 */
class plgPaytenpay extends Pay
{
	var $_pay = null;	//支付参数
	function plgPaytenpay( $params = null )
	{
 		$this->_pay = $params; 
	}
	...
	function display($out_trade_no,$subject,$body,$total_fee,$pay_mode="directPay")
	{
        ...
        /* 订单描述，用订单号替代 */
        if (!empty($out_trade_no['order_id']))
        {
            //$desc = get_goods_name_by_id($order['order_id']);
            $desc = $out_trade_no['order_sn'];
            $attach = '';
        } 
        ...
        /* 返回的路径 */
        $return_url = URI::base(); //return_url('tenpay');
        ...
        $button  = '<br /><form name="tenpay" style="text-align:center;" action="https://www.tenpay.com/cgi-bin/v1.0/pay_gate.cgi"   style="margin:0px;padding:0px" >';
        ...
        $button  .= '<input type="image" src=" images/tenpay.gif" value="去支付" /></form><br /><script>document.forms[\'tenpay\'].submit();</script>';
